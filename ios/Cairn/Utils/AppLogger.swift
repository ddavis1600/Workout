...
///   - captured in Console.app + sysdiagnose, so beta crash reports
///     actually include the trail.
///   - off-process — logging doesn't block the caller's actor, which
///     matters in the main-thread save paths we're instrumenting.
///   - respects Privacy filtering. We pass error descriptions as
///     `public` deliberately, since they don't include PII — just
///     SwiftData / CloudKit error codes.
enum AppLogger {
...
extension ModelContext {
    ...
    /// scraping stack frames. File/line are captured automatically via
    /// `#fileID` / `#line` for deep-link-in-Xcode jumping.
    ///
    /// Callers that need to react to a failure (toast, rollback) should
    /// use `try modelContext.save()` with do/catch directly. This
    /// helper is for the ~40 fire-and-forget sites where previously a
    /// `try?` swallowed the error entirely.
    @discardableResult
    func saveOrLog(
        _ label: String,
        file: String = #fileID,
