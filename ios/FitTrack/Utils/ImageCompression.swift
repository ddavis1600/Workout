...
///    callback — `UIImage.jpegData(compressionQuality:)` on a full-res
///    photo can cost 80–200 ms on A15-class hardware.
///
...
enum ImageCompression {
    ...
    /// Compress arbitrary image `Data` for storage. Runs on a detached
    /// user-initiated Task so the caller's actor (usually main) doesn't
    /// hitch.
