...
class HealthKitManager {
    ...
    /// Fetched bodyMass sample + the stable `HKQuantitySample.uuid`
    /// that produced it. The UUID is what callers persist on
    /// `WeightEntry.healthKitUUID` so re-imports don't double-count
    /// and HK-side edits flow back per sample identity.
